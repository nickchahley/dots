...
; Now draw the arrow tail along the path in the higher level function rather than a straight line here
        ...
        (if (= FullHead 1) (begin
            ...
            (gimp-image-select-polygon image CHANNEL-OP-REPLACE 8 points)
                                    ; selections are relative to the image origin, not the drawable
                                    ;  origin. If the drawable has an offset this will have been subtracted
                                    ;  from the points used to draw the arrow so here any offset must
                                    ;  be re-applied
            (gimp-selection-translate image (+ (car (gimp-drawable-offsets drawable)))
...
(define
    ...
    (let*
        ...
        (if (or (> major_version_no 2) (and (= major_version_no 2)
            (and (> minor_version_no 98) (> release_no 10))))
          (prog1
            (gimp-message "This script requires GIMP V2.99.10 or earlier to run.\nPlease use a later verson of the script\nfor GIMP versions 2.99.14 and later.\n(there is no version for V2.99.12)")
            (quit)
          ) ; end - prog1
        )   ; end - if
        ...
        (if (not (= theActiveVector -1)) (begin
            ...
            (set! x_offset (car (gimp-drawable-offsets drawable)))
            (set! y_offset (cadr (gimp-drawable-offsets drawable)))
            ...
            ; if DrawSolidBrush is FALSE the spacing, hardness, aspect ratio and
            ;  force set in the brush tool options are used - this can result in
            ;  the shaft of the arrow being a series of discontinuous points, rather
            ;  than a solid line if spacing is set to a high value. Similarly setting
            ;  force or hardness to lower values in the brush tool options will result
            ;  in a feinter arrow shaft. The setting of the aspect ratio can also affect
            ;  the appearance of the arrow

            (if (= 1 DrawSolidBrush)  ; if not to use the default brush settings
                (begin
...
(script-fu-register
    ...
    SF-TOGGLE      "Select to draw solid arrows, deselect to use the brush settings" TRUE
)
